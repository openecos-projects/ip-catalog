(() => {
  ...
  function filteredRecords() {
    ...
    const [field, direction] = state.sort.endsWith("_desc")
      ? [state.sort.slice(0, -5), "desc"]
      : [state.sort, "asc"];
    return records.sort((a, b) => {
      const left = rawText(a[field]);
      const right = rawText(b[field]);
  ...
  function sortFromTable(field) {
    if (state.sort === field) {
      state.sort = `${field}_desc`;
    } else if (state.sort === `${field}_desc`) {
      state.sort = field;
    } else {
      state.sort = field === "last_reviewed_at" ? `${field}_desc` : field;
    }
    if ([...els.sort.options].some((option) => option.value === state.sort)) {
